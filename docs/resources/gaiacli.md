...
`anekacli` is the tool that enables you to interact with the node that runs on the Cosmos Hub network, whether you run it yourself or not. Let us set it up properly. In order to install it, follow the [installation procedure](../aneka-tutorials/installation.md).

### Setting up anekacli

The main command used to set up `anekacli` is the following:
...
- `cosmos`
  - Derived from account keys generated by `anekacli keys add`
...
- `cosmosvalconspub`
  - Generated when the node is created with `anekad init`.
  - Get this value with `anekad tendermint show-validator`
...
Older versions of `anekacli` used store keys in the user's home directory. If you are migrating
from an old version of `anekacli` you will need to migrate your old keys into your operating system's
credentials storage by running the following command:

```bash
anekacli keys migrate
```
...
When broadcasting transactions, `anekacli` accepts a `--broadcast-mode` flag. This
...
```bash
anekacli tx send <sender_key_name_or_address> <recipient_address> 10faucetToken \
  --chain-id=<chain_id>
```
...
```bash
anekacli tx send <sender_key_name_or_address> <destination_cosmosaccaddr> 10faucetToken \
  --chain-id=<chain_id> \
  --dry-run
```
...
```bash
anekacli tx send <sender_address> <recipient_address> 10faucetToken \
  --chain-id=<chain_id> \
...
::: tip Note
The `--generate-only` flag prevents `anekacli` from accessing the local keybase.
...
where `[name]` is the name of the key you specified when you initialized `anekad`.
...
```bash
anekacli query distribution community-pool
```
...
```bash
anekacli tx send cosmos1570v2fq3twt0f0x02vhxpuzc9jc4yl30q2qned 1000000uatom \
