...
First, you need to [install the software](../aneka-tutorials/installation.md).

Then, you can start [running a full-node](../aneka-tutorials/join-mainnet.md).
...
```bash
anekacli keys add <your_key_name>
```
...
```bash
anekacli query account <YOUR_ADDRESS>
```
...
```bash
anekacli tx send <from_key_or_address> <to_address> <amount> \
...
```bash
anekacli rest-server --node=<full_node_address:full_node_port>
```
...
Every Cosmos SDK transaction has a canonical JSON representation. The `anekacli`
