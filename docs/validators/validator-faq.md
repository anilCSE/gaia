...
We view testnet participation as a great way to signal to the community that you are ready and able to operate a validator. You can find all relevant information about the testnet [here](../aneka-tutorials/join-testnet.md) and [here](https://github.com/cosmos/testnets).
...
- **Tendermint Key**: This is a unique key used to sign consensus votes.
  - It is associated with a public key `cosmosvalconspub` (Get this value with `anekad tendermint show-validator`)
  - It is generated when the node is created with anekad init.
- **Application key**: This key is created from `anekacli` and used to sign transactions. Application keys are associated with a public key prefixed by `cosmospub` and an address prefixed by `cosmos`. Both are derived from account keys generated by `anekacli keys add`.
