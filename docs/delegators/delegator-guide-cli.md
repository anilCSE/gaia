...
- [Installing `anekacli`](#installing-anekacli)
...
- [Setting Up `anekacli`](#setting-up-anekacli)
...
`anekacli`: This is the command-line interface to interact with a `anekad` full-node. 

::: warning
**Please check that you download the latest stable release of `anekacli` that is available**
:::

[**Download the binaries**]
Not available yet.

[**Install from source**](../aneka-tutorials/installation.md)

::: tip
`anekacli` is used from a terminal. To open the terminal, follow these steps:
...
```bash
anekacli keys add <yourKeyName> --recover
```
...
When you initialize your ledger, a 24-word mnemonic is generated and stored in the device. This mnemonic is compatible with Cosmos and Cosmos accounts can be derived from it. Therefore, all you have to do is make your ledger compatible with `anekacli`. To do so, you need to go through the following steps:
...
```bash
anekacli keys add <yourAccountName> --ledger 
```
...
```bash
anekacli keys add <yourKeyName> --recover --account 1
```
...
You will find the tutorial on how to install `anekad` [here](https://cosmos.network/docs/cosmos-hub/installation.html), and the guide to run a full-node [here](https://cosmos.network/docs/cosmos-hub/join-mainnet.html).

### Connecting to a Remote Full-Node

If you do not want or cannot run your own node, you can connect to someone else's full-node. You should pick an operator you trust, because a malicious operator could return  incorrect query results or censor your transactions. However, they will never be able to steal your funds, as your private keys are stored locally on your computer or ledger device. Possible options of full-node operators include validators, wallet providers or exchanges. 

In order to connect to the full-node, you will need an address of the following form: `https://77.87.106.33:26657` (*Note: This is a placeholder*). This address has to be communicated by the full-node operator you choose to trust. You will use this address in the [following section](#setting-up-anekacli).

## Setting Up `anekacli`

::: tip
**Before setting up `anekacli`, make sure you have set up a way to [access the Cosmos Hub network](#accessing-the-cosmos-hub-network)**
:::

::: warning
**Please check that you are always using the latest stable release of `anekacli`**
:::

`anekacli` is the tool that enables you to interact with the node that runs on the Cosmos Hub network, whether you run it yourself or not. Let us set it up properly.

In order to set up `anekacli`, use the following command:

```bash
anekacli config <flag> <value>
```
...
```bash
anekacli config node <host>:<port

// example: anekacli config node https://77.87.106.33:26657
```
...
```bash
anekacli config trust-node false
...
::: tip
**Before you can bond atoms and withdraw rewards, you need to [set up `anekacli`](#setting-up-anekacli)**
:::

`anekacli` lets you query all relevant information from the blockchain, like account balances, amount of bonded tokens, outstanding rewards, governance proposals and more. Next is a list of the most useful commands for delegator. 

```bash
// query account balances and other account-related information
anekacli query account <yourAddress>
...
anekacli query staking delegation <delegatorAddress> <validatorAddress>

// query the rewards of a delegator given a delegator address (e.g. cosmos10snjt8dmpr5my0h76xj48ty80uzwhraqalu4eg)
anekacli query distribution rewards <delegatorAddress> 

// query all proposals currently open for depositing
anekacli query gov proposals --status deposit_period

// query all proposals currently open for voting
anekacli query gov proposals --status voting_period

// query a proposal given its proposalID
anekacli query gov proposal <proposalID>
```
...
::: tip
**Before you can bond atoms and withdraw rewards, you need to [set up `anekacli`](#setting-up-anekacli) and [create an account](#creating-an-account)**
:::
...
anekacli tx send <to_address> <amount> --from <yourKeyName> --gas auto --gas-adjustment 1.5 --gas-prices <gasPrice>
```
...
anekacli tx staking delegate <validatorAddress> <amountToBond> --from <delegatorKeyName> --gas auto --gas-adjustment 1.5 --gas-prices <gasPrice>
...
anekacli tx staking redelegate <srcValidatorAddress> <destValidatorAddress> <amountToRedelegate> --from <delegatorKeyName> --gas auto --gas-adjustment 1.5 --gas-prices <gasPrice>

// Withdraw all rewards
// ex value for flag: <gasPrice>=0.025uatom

anekacli tx distribution withdraw-all-rewards --from <delegatorKeyName> --gas auto --gas-adjustment 1.5 --gas-prices <gasPrice>
...
anekacli tx staking unbond <validatorAddress> <amountToUnbond> --from <delegatorKeyName> --gas auto --gas-adjustment 1.5 --gas-prices <gasPrice>
```
...
anekacli tx gov submit-proposal --title "Test Proposal" --description "My awesome proposal" --type <type> --deposit=10000000uatom --gas auto --gas-adjustment 1.5 --gas-prices <gasPrice> --from <delegatorKeyName>

// Increase deposit of a proposal
// Retrieve proposalID from $anekacli query gov proposals --status deposit_period
// ex value for parameter: <deposit>=10000000uatom

anekacli tx gov deposit <proposalID> <deposit> --gas auto --gas-adjustment 1.5 --gas-prices <gasPrice> --from <delegatorKeyName>

// Vote on a proposal
// Retrieve proposalID from $anekacli query gov proposals --status voting_period 
// <option>=yes/no/no_with_veto/abstain

anekacli tx gov vote <proposalID> <option> --gas auto --gas-adjustment 1.5 --gas-prices <gasPrice> --from <delegatorKeyName>
```
...
anekacli tx staking delegate <validatorAddress> <amountToBond> --from <delegatorAddress> --gas auto --gas-adjustment 1.5 --gas-prices <gasPrice> --generate-only > unsignedTX.json
```
...
```bash
anekacli query account <yourAddress> --chain-id cosmoshub-2
```
...
```bash
anekacli tx sign unsignedTx.json --from <delegatorKeyName> --offline --chain-id cosmoshub-2 --sequence <sequence> --account-number <account-number> > signedTx.json
```
