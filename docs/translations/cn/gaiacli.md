...
`anekacli`是一个工具，使您能够与 Cosmos Hub 网络中的节点进行交互，无论您是否自己运行它。 让我们恰当的设置它。 要安装它，请按照[安装步骤](./ installation.md)进行安装。
...
+ `cosmos` 
	+ 从通过`anekacli keys add`生成的账户私钥中产生
	+ 用于接收资金
...
+ `cosmosvalconspub`
	+ 在使用`anekad init`创建节点时生成
	+ 使用`anekad tendermint show-validator`获得该值
...
```bash
anekacli keys add <account_name>
```
...
```bash
anekacli keys show <account_name>
```
...
在广播交易时，`anekacli`接受`--broadcast-mode`标识。 这个标识的值可以是`sync`（默认值）、`async`或`block`，其中`sync`使客户端返回 CheckTx 响应，`async`使客户端立即返回，而`block`使得 客户端等待 tx 被提交（或超时）。
...
```bash
anekacli tx send ... --fees=50000uatom
```
...
```bash
anekacli query account <account_cosmos>
```
...
```bash
anekacli tx send <destination_cosmos> 10faucetToken \
  --chain-id=<chain_id> \
  --from=<key_name> 
```
...
```bash
anekacli query account <account_cosmos>
anekacli query account <destination_cosmos>
```

你还可以使用`--block`标识查询在特定高度区块下你的余额：

```bash
anekacli query account <account_cosmos> --block=<block_height>
```

你可以通过在命令行中附加`--dry-run`标识来模拟交易而不实际广播它：

```bash
anekacli tx send <destination_cosmosaccaddr> 10faucetToken \
  --chain-id=<chain_id> \
  --from=<key_name> \
  --dry-run
```
...
```bash
anekacli tx sign --validate-signatures signedSendTx.json
```
...
```bash
anekacli tx slashing unjail --from <validator-operator-addr>
```
...
```bash
anekacli query staking validator <account_cosmosval>
```
...
```bash
anekacli query staking delegation <delegator_addr> <validator_addr>
```

或者你想查看所有当前的委托：

```bash
anekacli query staking delegations <delegator_addr>
```
...
```bash
anekacli query staking unbonding-delegation <delegator_addr> <validator_addr>
```

或者你可以查看当前你所有的unbonding-delegation:

```bash
anekacli query staking unbonding-delegations <account_cosmos>
```

此外，你可以从特定验证人获取所有unbonding-delegation：

```bash
anekacli query staking unbonding-delegations-from <account_cosmosval>
```
...
```bash
anekacli query staking redelegation <delegator_addr> <src_val_addr> <dst_val_addr>
```

或者，如果你可以检查所有当前的unbonding-delegation：

```bash
anekacli query staking redelegations <account_cosmos>
```

此外，你可以查询某个特定验证人的所有转出的重新绑定：

```bash
anekacli query staking redelegations-from <account_cosmosval>
```
...
```bash
anekacli query delegations-to <account_cosmosval>
```
...
```bash
anekacli tx gov submit-proposal param-change <path/to/proposal.json> \
  --from=<name> \
  --chain-id=<chain_id>
```
...
```bash
anekacli query gov proposal <proposal_id>
```
...
```bash
anekacli query gov proposer <proposal_id>
```
...
```bash
anekacli tx gov deposit <proposal_id> "10000000uatom" \
  --from=<name> \
  --chain-id=<chain_id>
```
...
```bash
anekacli query gov deposits <proposal_id>
```

你还可以查询特定地址提交的存入金：

```bash
anekacli query gov deposit <proposal_id> <depositor_address>
```
...
```bash
anekacli tx gov vote <proposal_id> <Yes/No/NoWithVeto/Abstain> \
  --from=<name> \
  --chain-id=<chain_id>
```
...
```bash
anekacli query gov vote <proposal_id> <voter_address>
```

你还可以查询提交给所有此前投给指定提案的投票：

```bash
anekacli query gov votes <proposal_id>
```
...
```bash
anekacli query gov tally <proposal_id>
```
...
```bash
anekacli query distribution commission <validator_address>
```
...
```bash
anekacli query distribution slashes <validator_address> <start_height> <end_height>
```
...
```bash
anekacli query distribution rewards <delegator_address> <validator_address>
```
...
```bash
anekacli query distribution rewards <delegator_address>
```
...
可以通过完全命令生成主流的UNIX shell解释器（如`Bash`和`Zsh`）的`completion`命令，该命令可用于`anekad`和`anekacli`。
